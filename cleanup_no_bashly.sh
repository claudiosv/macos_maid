#!/usr/bin/env bash
#
# macOS deep cleanup and update utility
...
FLAGS:
  -h  Show help
  -m  Do not install macOS updates
  -l  Clear Launchpad layout
  -c  Clear Chrome
  -i  Clear iOS/iPadOS local backups
  -n  Dry-run (show what would be deleted without doing it)
  -v  Verbose output (prints executed commands and removed paths)
  -o  Log execution output
EOF
}

while getopts ":mhlcinvo" opt; do
	case "${opt}" in
	m) DO_UPDATES=false ;;
	l) CLEAR_LAUNCHPAD=true ;;
	c) CLEAR_CHROME=true ;;
	i) CLEAR_IOS=true ;;
	n) DRY_RUN=true ;;
	v) VERBOSE=true ;;
...
bytes_to_human() {
	local delta="${1:-0}" abs unit i frac
...
if require_cmd softwareupdate; then
	...
	if grep -Fq "No new software available" <<<"${su_output}"; then
		...
		if ${DO_UPDATES}; then
			...
			sparkle "macOS updates are available, but DO_UPDATES is false. Skipping..."
		fi
	fi
fi
...
if ((${#electron_apps[@]} > 0)); then
	...
	sparkle "Discovered apps: ${unique_apps[*]}"
	for app_path in "${unique_apps[@]}"; do
		# Extract just the app folder name (e.g., "Slack" or "Default")
		app_name="$(basename "${app_path}")"

		# Chromium profile edge-case: If the folder is "Default", use the parent directory name
		if [[ "${app_name}" == "Default" ]]; then
			app_name="$(basename "$(dirname "${app_path}")")"
		fi

		clean_electron_app "${app_path}" "${app_name}"
	done
else
